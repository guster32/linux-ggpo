...
void SyncTestBackend::RaiseSyncError(const char *fmt, ...) {
   ...
   puts(buf);
   EndLog();
   assert(0 && "Synchronization error encountered.");
}
...
void SyncTestBackend::BeginLog(int saving) {
   ...
   mkdir("synclogs", 0755);
   snprintf(filename, sizeof(filename), "synclogs/%s-%04d-%s.log",
            saving ? "state" : "log",
            _sync.GetFrameCount(),
...
void SyncTestBackend::LogSaveStates(SavedInfo &info) {
   char filename[PATH_MAX];
   snprintf(filename, sizeof(filename), "synclogs/state-%04d-original.log", _sync.GetFrameCount());
   _callbacks.log_game_state(filename, (unsigned char *)info.buf, info.cbuf);

   snprintf(filename, sizeof(filename), "synclogs/state-%04d-replay.log", _sync.GetFrameCount());
